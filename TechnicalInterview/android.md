...
## 비동기 처리와 Reactive Programming

컴퓨터 프로그램에는 **세가지** 종류가 있다.

1. 주어진 입력 값을 바탕으로 결과를 계산하는 변환 프로그램으로 일반적으로 컴파일러와 수치계산을 하는 프로그램

2. 프로그램이 작업을 주도하며 사용자 혹은 다른 프로그램과 상호작용을 하는 프로그램.

3. **프로그램 자신의 주변환경과 끊임없이 상호작용을 하는데 프로그램이 주도하는 것이 아니라 환경이 변하면 이벤트를 받아 동작하는 Reactive 프로그램**

이중 세번째 프로그램에 해당하는 **Reactive Program** 에 대해 알아보려고 한다.

### Reactive Programming이 필요한 이유?

개발자들은 사용자 경험(UX)을 향상시키고 싶어한다. 그래서 앱에서 메인스레드가 멈추거나 느려지지 않도록 해야하며 사용자들에게 부드러운 UX와 좋은 앱 성능을 제공하고 싶어한다.  

하지만 ***메인스레드*** 를 자유롭게 조작하면서 유지하려면 무겁고 시간이 오래걸리는 작업은 ***백그라운드*** 에서 동작해야 한다. 그리고 백그라운드에서 조차 무겁고 복잡한 계산 작업이라면 ***서버*** 에서 수행하는 것이 Best이다. 따라서 네트워크 운영을 위해서는 **비동기 작업** 이 필요하다.

### AyncTask 쓰면 돼?

AsyncTask는 2019년 11월 8일 Deprecated 되었다. 다른 비동기 처리 라이브러리를 써야한다.

***왜?***

많은 어플리케이션이 대표적으로 가지는 문제 중 하나는 서버로 부터 데이터를 가져오는 길고 긴 백그라운드 작업이다. 네트워크 작업은 시간이 짧다면 상관이 없지만 오래걸리는 상황의 경우에는 비동기적으로 서버에 요청하고 데이터를 받아서 UI 업데이트를 하는 것이 중요하다.

하지만 네트워크 요청이 완료될 때 UI와 관련된 부분에 어떠한 문제가 있어 더 이상 존재하지 않거나 에러가 발생하여 충돌 또는 버그가 발생할 수 있는 근본적인 문제가 있다. 이건 일반적인 스레드 프로그래밍이 가지고 있는 위험성과 크게 다르지 않다.

AsynckTask는 전체적인 프로세스를 단순화하지만 **안드로이드의 생명주기를 신경쓰지 않는다.**

그렇기 때문에 액티비티나 프레그먼트가 안드로이드 생명주기에 의해 재생성되거나 파괴되었을 때 마무리 작업에 대한 내용이 보호되지 않는 불편한 점이 있다.

***아하... 그럼?***

그래서 나온 것이 **ReactiveX** 이다.

**ReactiveX** 는 비동기 프로그래밍과 Observable 시퀀스를 이용해 이벤트를 처리하기 위한 라이브러리이다.

사실 현재 많은 **Reactive** 관련 라이브러리가 나와있는데 대부분 **ReactiveX** 를 사용하기 때문에 RxKotlin, RxJava, RxAndroid, RxSwift 등 들은 서로 다른 것이 아니라 하나의 **ReactiveX Extensions** 인 것이다.
